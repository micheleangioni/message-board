...
if(config('ma_messageboard.api.v1_enabled')) {
    ...
    Route::group(['namespace' => 'MicheleAngioni\\MessageBoard\\Http\\Controllers\\Api\\v1', 'prefix' => config('ma_messageboard.api.api_prefix') . '/v1'], function() {

        // Authentication

        if (config('ma_messageboard.api.authentication')) {
            Route::post('auth', array('as' => 'mb.api.auth.authenticate', 'uses' => 'AuthenticationController@authenticate'));
        }

        Route::group(['middleware' => 'jwt.auth'], function () {
            // Logout
            Route::delete('auth', array('as' => 'mb.api.auth.logout', 'uses' => 'AuthenticationController@logout'));
